...
- Vexa selector reference (`data-audio-level`, `[aria-label="Participants"]`, panel button fallbacks) — https://github.com/Vexa-ai/vexa/blob/main/services/vexa-bot/core/src/platforms/googlemeet/selectors.ts
- meetingbot/meetingbot Meet bot (People-panel observer, `data-participant-id`, `data-requested-participant-id`) — https://github.com/meetingbot/meetingbot/blob/main/src/bots/meet/src/bot.ts
- Recall.ai Meet bot blog (caption MutationObserver pattern) — https://www.recall.ai/blog/how-i-built-an-in-house-google-meet-bot
- Sarvam STT REST reference (`with_timestamps`, `timestamps` shape) — https://docs.sarvam.ai/api-reference-docs/speech-to-text/transcribe
- Sarvam Saaras model page (v3 timestamp confirmation) — https://docs.sarvam.ai/api-reference-docs/getting-started/models/saaras
- Sarvam Batch API (alternative `with_diarization` path, kept as fallback option only) — https://docs.sarvam.ai/api-reference-docs/api-guides-tutorials/speech-to-text/batch-api
